export class StringUtils {
    ...
    ;
    static orderByLevenshteinDistance(text, words) {
        const ordered = [];
        words.sort((a, b) => {
     ...
     * @returns True if the text contains the searcher
     */
    static containsMatchingWithTolerance(text, searcher, tolerance) {
     ...
     * @return
     */
    static getMatching(text, searcher) {
     ...
     * @return
     */
    static getMatchingWithTolerance(text, searcher, tolerance) {
        const words = text.split(/\s/);
        let matchingWord = '';
        words.forEach(word => {
...
StringUtils.DEFAULT_TOLERANCE = 2;
