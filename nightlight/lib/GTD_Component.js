import { resolveAfterMs } from "./GTD_TimeTools.js";

//MAKE A NEW COMPONENT
export function create(properties) {
  if (!properties) properties = {};
  if (!properties.element) properties.element = document.createElement("div");

  //inner functions
  properties.appendChild = (e) => properties.element.appendChild(e);
  properties.removeChild = (e) => properties.element.removeChild(e);
  properties.appendTo = (e) => e.appendChild(properties.element);
  properties.clean = () => (properties.element.innerHTML = "");
  properties.show = (time) => resolveAfterMs(() => setStyles(properties.element, { opacity: 1 }) , time ? time : 100);
  properties.hide = (time) => resolveAfterMs(() => setStyles(properties.element, { opacity: 0 }) , time ? time : 100);

  //check properties
  if (properties.type) properties.element = document.createElement(properties.type);
  if (properties.id) properties.element.id = properties.id;
  if (properties.text) properties.element.innerHTML = properties.text;
